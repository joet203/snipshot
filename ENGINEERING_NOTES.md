...
- Content-trigger jobs are **one-shot**, and re-arming uses **two alternating job ids** (`JOB_ID_A`/`JOB_ID_B`): `onStartJob` arms the OTHER id before processing, so a trigger stays registered for the whole batch window. Same-id re-arm is forbidden — see gotcha #13. After each batch, an unconditional `queryFreshScreenshots` sweep catches anything that landed in the ms gap before `armNext` registered (e.g. an `IS_PENDING` flip mid-processing). `jobFinished` is routed through `mainHandler` AFTER the overlay `show()` posts, so the overlay window is attached before the job's process-priority guarantee is released.
- `stop()` cancels both ids AND flips a volatile `enabled` gate checked at job entry and immediately before `OverlayManager.show` — canceling jobs does not stop already-posted work, so without the gate an overlay could appear after Stop.
...
13. **`JobScheduler.schedule()` with the id of a currently-RUNNING job STOPS that job** — verbatim from the SDK javadoc ("If a job with the given ID is currently running, it will be stopped"). So a content-trigger job must never re-arm its own id from `onStartJob`; v0.2.0 did exactly that and every fire self-stopped (found in the v0.2.1 review, flagged independently by codex and Claude). Fix: alternate `JOB_ID_A`/`JOB_ID_B` — the fired job arms the other id. Also check the return value: `schedule()` can return `RESULT_FAILURE` (quota/throttling) without throwing; `onJobFired`'s `finally` self-heals by re-arming if nothing is pending.
